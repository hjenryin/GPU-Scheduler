"""Unit tests for scheduler.core.singleton module"""
import os
import json
import pytest
import signal
...
class TestSingletonDaemon:
    ...
    def test_release_lock_with_exception(self, singleton):
        ...
        with patch('scheduler.core.singleton.os.close', side_effect=OSError("test error"), autospec=True):
            # Should not raise
            singleton.release_lock()
...
class TestIsDaemonRunning:
    ...
    def test_missing_file_after_check(self, temp_lockfile):
        ...
        with patch('scheduler.core.singleton.os.path.exists', return_value=False, autospec=True):
            assert is_daemon_running(temp_lockfile) is False
