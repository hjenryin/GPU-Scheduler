...
class TestWorkerDaemon:
    ...
    @patch('scheduler.worker.daemon.GPUMonitor', autospec=True)
    @patch('scheduler.worker.daemon.FileHandler', autospec=True)
    def test_cleanup_skips_jobs_with_reported_terminal_status(
        self, mock_file_handler, mock_gpu_monitor, mock_job_executor,
        mock_heartbeat, mock_client, test_config
    ):
        """Test that cleanup mechanism skips jobs already reported as complete/failed"""
        ...
        mock_job.job_id = 'job-001'
        mock_job.command = ['sleep', '2']

        import threading
        mock_thread = Mock(spec=threading.Thread)

        # Setup scenario: Job completed and reported 60 seconds ago (past grace period)
        # but still in active_jobs (waiting for poll response cleanup)
        daemon.active_jobs = {
            'job-001': {
                'job': mock_job,
                'pid': 12345,
                'start_time': datetime.now() - timedelta(seconds=60),  # Old enough
                'monitor_thread': mock_thread,
                'reported_status': 'completed'  # Already reported as completed
            }
        }

        # Call cleanup with empty running_job_ids (job should be in this list if running)
        daemon._handle_cleanup_request(recorded_job_ids=['job-001'], running_job_ids=[])

        # Assert: Job NOT terminated
        mock_executor_instance.terminate_job.assert_not_called()

        # Assert: Job NOT reported as failed again
        mock_client_instance.report_job_failed.assert_not_called()

        # Assert: Job still in active_jobs (will be removed by poll response)
        assert 'job-001' in daemon.active_jobs
        assert daemon.active_jobs['job-001']['reported_status'] == 'completed'
    ...
    @patch('scheduler.worker.daemon.GPUMonitor', autospec=True)
    @patch('scheduler.worker.daemon.FileHandler', autospec=True)
    def test_cleanup_terminates_jobs_without_reported_status(
        self, mock_file_handler, mock_gpu_monitor, mock_job_executor,
        mock_heartbeat, mock_client, test_config
    ):
        """Test that cleanup terminates jobs that haven't been reported (after grace period)"""
        from datetime import datetime, timedelta
        ...
        mock_job.command = ['sleep', '100']

        import threading
        mock_thread = Mock(spec=threading.Thread)

        # Setup scenario: Job NOT reported, past grace period, not in running_job_ids
        daemon.active_jobs = {
            'job-002': {
                'job': mock_job,
                'pid': 12346,
                'start_time': datetime.now() - timedelta(seconds=35),  # Past 30s grace period
                'monitor_thread': mock_thread,
                'reported_status': None  # NOT reported yet
            }
        }

        # Call cleanup with empty running_job_ids
        daemon._handle_cleanup_request(recorded_job_ids=['job-002'], running_job_ids=[])

        # Assert: Job WAS terminated
        mock_executor_instance.terminate_job.assert_called_once_with(12346)

        # Assert: Job WAS reported as failed
        mock_client_instance.report_job_failed.assert_called_once_with(
            'job-002', "Job cancelled by head node"
        )

        # Assert: Job removed from active_jobs
        assert 'job-002' not in daemon.active_jobs
